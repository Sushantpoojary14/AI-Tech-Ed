...
const ProfileEditModal = () => {
    const { handlePEClose,openPE ,handlePE2Open, handlePE2Close, dataSubmit, dispatch, handleSuccessOpen ,openPE2,handlePESuccessOpen} = UserContext();
    ...
    return (
        <Dialog onClose={handlePEClose} open={openPE} >
            <Box sx={{
                width: { lg: '482px', md: '482px', sm: '482px', xs: '320px' }, height: '700px', display: 'flex', flexDirection: 'column', px: '40px'
            }}>
                ...
                <form onSubmit={handleSubmit(onSubmit)} >
                    <Stack spacing={4} direction="column" margin="auto">
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                            <Input label="Name" val={false} type="text"
                                reg={register("name")} defaultVal={data.firstName}/>
                            {/* {errors.name &&
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                            <Input label="User Id" val={false} type="text"
                                reg={register("user_id")} defaultVal={data.id}/>
                            {/* {errors.new_password &&
                                <Typography sx={{ mt: 3, p: 0, color: 'red' }}>
                                    *Minimum 8 - Maximum 16 character
                                </Typography>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                            <Input label="Birth Date" val={false} type="date"
                                reg={register("birth_date")} defaultVal={data.birthDate}/>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                            <Input label="Email" val={false} type="email"
                                reg={register("email")} defaultVal={data.email}/>
                            {/* {errors.confirm_password &&
                                <Typography sx={{ mt: 3, p: 0, color: 'red' }}>
                                    {errors.new_password?.type == 'validate' ? '*Minimum 8 - Maximum 16 character' :
                                        'Passwords do not match'}
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                            <Input label="Phone Number" val={false} type="telephone"
                                reg={register("phone")} defaultVal={data.phone}/>
                            {/* {errors.confirm_password &&
                                <Typography sx={{ mt: 3, p: 0, color: 'red' }}>
                                    {errors.new_password?.type == 'validate' ? '*Minimum 8 - Maximum 16 character' :
                                        'Passwords do not match'}
                        ...
                        <Stack spacing={{ lg: 6, md: 6, sm: 6, xs: 2 }} direction="row" marginX="auto">
                            <WButton name="CANCEL" css={{ width: '177px' }} func={handlePEClose} />
